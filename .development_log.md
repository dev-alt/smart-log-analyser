...
**User Request**: "on github we can see sensitive patterns in PATTERNS=( \"[DOMAIN]\" \"[IP_ADDRESS]\" \"ssh-rsa AAAA\" \"BEGIN PRIVATE KEY\" \"BEGIN RSA PRIVATE KEY\" ) in /check-sensitive-data.sh and in dev log ./smart-log-analyser analyse downloads/[SENSITIVE_PATH], remember to also prune this from git history"
