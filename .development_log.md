...
5. ✅ **Security Visualizations & Reporting** (`pkg/security/visualization.go`)
...
**Enhanced HTML Generator (`pkg/html/generator.go`)**:
...
**User Request**: Communication architecture for Go application to Avalonia C# dashboard integration

**Requirements Analysis**:
- ✅ **Communication Method Selection**: User initially preferred HTTP, then corrected to prefer non-HTTP solutions
- ✅ **Cross-Platform Compatibility**: Required support for Windows and Linux deployment scenarios
- ✅ **Dashboard Integration**: Specific focus on C# Avalonia desktop application communication

**IPC Architecture Implementation**:

**🔧 Core IPC System (pkg/ipc/)**:
```go
// Cross-platform IPC server with automatic protocol selection
type Server struct {
    listener   net.Listener    // Named Pipes (Windows) / Unix Sockets (Linux/macOS)
    clients    map[string]net.Conn  // Multi-client connection management
    analyzer   *analyser.Analyser   // Full analysis engine integration
    configMgr  *config.Manager      // Configuration management integration
    htmlGen    *html.Generator      // Report generation integration
    queryExec  *query.Executor      // SLAQ query execution integration
}
```

**🌐 Cross-Platform Communication**:
- **Windows**: Named Pipes (`\\.\pipe\SmartLogAnalyser`) for native Windows IPC
- **Linux/macOS**: Unix Domain Sockets (`/tmp/smart-log-analyser.sock`) for POSIX systems
- **Automatic Protocol Selection**: Runtime detection using `runtime.GOOS`
- **JSON Message Protocol**: Simple request/response format for cross-language compatibility
...
// Request Format
{
    ...
    "action": "analyze|query|listPresets|runPreset|getConfig|updateConfig|getStatus|shutdown",
    "logFile": "/path/to/access.log",
    "options": { "enableSecurity": true, "generateHtml": true, "interactive": true },
    "query": "SELECT * FROM logs WHERE status_code >= 400",
    "preset": "security_overview"
}
...
// Response Format  
{
    "id": "unique-request-id",
    "success": true,
    "data": { /* analysis results, query results, configuration data */ },
    "error": "error message if applicable"
}
```

**🔄 Supported Operations**:
1. **`analyze`** - Complete log analysis with all Smart Log Analyser features
2. **`query`** - Execute SLAQ queries for custom analysis
3. **`listPresets`** - Retrieve available analysis presets (12 built-in presets)
4. **`runPreset`** - Execute specific analysis preset
5. **`getConfig`** - Get current configuration and templates
6. **`updateConfig`** - Update system configuration
7. **`getStatus`** - Server status and client connection information
8. **`shutdown`** - Graceful server shutdown

**⚡ CLI Integration (cmd/server.go)**:
```bash
# Start IPC server with professional output
...
**Cross-Platform Client (SmartLogAnalyserClient.cs)**:
```csharp
public class SmartLogAnalyserClient : IDisposable
{
    ...
    // Full API surface for dashboard integration
    public async Task<AnalysisResult> AnalyzeAsync(string logFile, AnalysisOptions options)
    public async Task<QueryResult> QueryAsync(string logFile, string query)  
    public async Task<PresetListResult> ListPresetsAsync()
    public async Task<QueryResult> RunPresetAsync(string logFile, string presetName)
    public async Task<StatusResult> GetStatusAsync()
}
```

**📊 Dashboard Integration Features**:
- **Strongly-Typed Data Models**: Complete C# classes matching Go structs
- **Async/Await Support**: Non-blocking operations for responsive UI
- **Connection Management**: Automatic connection handling with error recovery
- **Example Implementation**: Complete working example with project file

**🔧 Advanced Features**:
- **Multi-Client Support**: Concurrent dashboard connections with isolated request handling
- **Graceful Shutdown**: Clean server termination with client notification
- **Error Handling**: Comprehensive error reporting with detailed error messages
- **Request Tracking**: Unique request IDs for request/response correlation
- **Platform Abstraction**: Transparent communication regardless of underlying platform

**📁 File Structure**:
```
pkg/ipc/
├── types.go           # Request/response structures and constants
├── server.go          # Cross-platform IPC server implementation
cmd/
├── server.go          # CLI server command with professional output
examples/csharp/
├── SmartLogAnalyserClient.cs  # Complete C# client library
├── Program.cs         # Working example implementation
└── SmartLogAnalyser.csproj    # .NET project configuration
```

**📚 Documentation Updates**:
- ✅ **README.md**: Added comprehensive IPC Server & Dashboard Integration section
- ✅ **Command Reference**: Added `server` command documentation with full feature list
- ✅ **Integration Examples**: C# code examples and usage scenarios
- ✅ **Operation Table**: Complete IPC action reference with use cases

**🧪 Testing & Validation**:
- ✅ Cross-platform IPC server compiles and starts correctly
- ✅ JSON protocol handles all supported operations
- ✅ C# client library provides complete API surface
- ✅ Multi-client connection management works properly
- ✅ Graceful shutdown preserves data integrity
- ✅ Error handling provides useful diagnostic information

**Session Impact**: This implementation provides a complete IPC communication solution for integrating Smart Log Analyser with external dashboard applications. The cross-platform approach ensures compatibility across Windows and Linux deployments, while the comprehensive C# client library enables rapid Avalonia dashboard development. The JSON-based protocol is simple yet powerful, providing access to all Smart Log Analyser features including the 12 analysis presets, SLAQ query execution, interactive HTML report generation, and configuration management.

**Current Status**: Smart Log Analyser now includes enterprise-grade IPC capabilities, enabling seamless integration with desktop dashboard applications while maintaining the powerful CLI and interactive menu interfaces. The system supports concurrent multi-client connections and provides a professional foundation for building sophisticated dashboard applications that leverage Smart Log Analyser's comprehensive log analysis capabilities.
